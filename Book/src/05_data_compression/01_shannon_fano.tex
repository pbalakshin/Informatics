% !TeX root = ../../main.tex
\subsubsection{Алгоритм Шеннона-Фано}\label{subsubsec:shannon-fano}

\begin{figure}[H]
    \begin{minipage}[c]{0.25\textwidth}
        \centering
        \includegraphics[width=0.9\linewidth]{person/fano-robert}
        \captionof*{figure}{Роберт Фано\\1917--2016}
    \end{minipage}%
    \begin{minipage}[c]{0.75\textwidth}
        Алгоритм Шеннона-Фано --- один из первых алгоритмов сжатия.
        Его сформулировали два ученых --- Клод Шеннон и Роберт Фано.
        Алгоритм основан на частоте повторения.
        Так, часто встречающийся символ кодируется кодом меньшей длины, а редко встречающийся --- кодом большей длины.
        Коды, полученные при кодировании, префиксные, что позволяет декодировать любую последовательность.

        \medskip
        Алгоритм Шеннона-Фано для некоторых последовательностей может сформировать неоптимальные коды.
    \end{minipage}
\end{figure}

\bigskip
\paragraph{Описание алгоритма Шеннона-Фано}\label{par:shannon-fano-description}
\begin{enumerate}
    \item Символы входного (первичного) алфавита выписывают по убыванию вероятностей --- это корень будущего дерева;
    \item Строится дерево от корня к листьям. Находится середина, которая делит корень на два узла. Эти узлы (суммы вероятностей символов алфавита) примерно равны;
    \item Полученные узлы --- листья дерева. Левому узлу (с большей суммарной вероятностью) присваивается значение $1$, а правому --- $0$;
    \item Шаги 2--3 повторяются, пока в листьях дерева не останется один символ первичного алфавита.
    \item Символ входного (первичного) алфавита кодируется последовательностью нулей и единиц в соответствии с распределением их от корня к листьям (узлам).
\end{enumerate}

\begin{example}
    \task%
    Составить код Шеннона-Фано для последовательности $AAABCCCCCDEEEF$. Найти среднюю длину кодового слова.

    \solution%
    В последовательности $AAABCCCCCDEEEF$ алфавит состоит из 6 символов: A, B, C, D, E, F. Выпишем символы первичного алфавита по убыванию вероятностей:
    \begin{table}[H]
        \centering
        \begin{tabular}{|r|c|c|c|c|c|c|}
            \hline
            \textbf{Символ} & C & A & E & B & D & F \\ \hline
            \textbf{Вероятность} & $\sfrac{5}{14}$ & $\sfrac{3}{14}$ & $\sfrac{3}{14}$ & $\sfrac{1}{14}$ & $\sfrac{1}{14}$ & $\sfrac{1}{14}$ \\ \hline
        \end{tabular}
    \end{table}

    Полученная последовательность $CAEDBF$ является корнем будущего дерева.

    Построим дерево от корня к листьям:

    \begin{table}[H]
        \centering
        \begin{tabular}{cccccc}
            \multicolumn{6}{c}{CAEBDF ($\sfrac{5}{14} + \sfrac{3}{14} + \sfrac{3}{14} + \sfrac{1}{14} + \sfrac{1}{14} + \sfrac{1}{14}$)} \\
            % \multicolumn{6}{c}{CAEBDF ($\frac{5}{14} + \frac{3}{14} + \frac{3}{14} + \frac{1}{14} + \frac{1}{14} + \frac{1}{14}$)} \\
            \multicolumn{2}{c}{$\swarrow$} & \multicolumn{4}{c}{$\searrow$} \\
            \multicolumn{2}{c}{CA ($\sfrac{5}{14} + \sfrac{3}{14}$)} & \multicolumn{4}{c}{EBDF ($\sfrac{3}{14} + \sfrac{1}{14} + \sfrac{1}{14} + \sfrac{1}{14}$)} \\
            % \multicolumn{2}{c}{CA ($\frac{5}{14} + \frac{3}{14}$)} & \multicolumn{4}{c}{EBDF ($\frac{3}{14} + \frac{1}{14} + \frac{1}{14} + \frac{1}{14}$)} \\
            $\swarrow$ & $\searrow$ & \multicolumn{2}{c}{$\swarrow$} & \multicolumn{2}{c}{$\searrow$} \\
            C ($\sfrac{5}{14}$) & A ($\sfrac{3}{14}$) & \multicolumn{2}{c}{EB ($\sfrac{3}{14} + \sfrac{1}{14}$)} & \multicolumn{2}{c}{DF ($\sfrac{1}{14} + \sfrac{1}{14}$)} \\
            & & $\swarrow$ & $\searrow$ & $\swarrow$ & $\searrow$ \\
            & & E ($\sfrac{3}{14}$) & B ($\sfrac{1}{14}$) & D ($\sfrac{1}{14}$) & F ($\sfrac{1}{14}$) \\
        \end{tabular}
    \end{table}

    Присвоим левому символу (с большей вероятностью) значение $1$, а правому --- $0$:

    \begin{table}[H]
        \centering
        \begin{tabular}{cccccc}
            \multicolumn{6}{c}{CAEBDF ($\sfrac{5}{14} + \sfrac{3}{14} + \sfrac{3}{14} + \sfrac{1}{14} + \sfrac{1}{14} + \sfrac{1}{14}$)} \\
            \multicolumn{2}{c}{\textbf{[1]} $\swarrow$} & \multicolumn{4}{c}{$\searrow$ \textbf{[0]}} \\
            \multicolumn{2}{c}{CA ($\sfrac{5}{14} + \sfrac{3}{14}$)} & \multicolumn{4}{c}{EBDF ($\sfrac{3}{14} + \sfrac{1}{14} + \sfrac{1}{14} + \sfrac{1}{14}$)} \\
            \textbf{[1]} $\swarrow$ & $\searrow$ \textbf{[0]} & \multicolumn{2}{c}{\textbf{[1]} $\swarrow$} & \multicolumn{2}{c}{$\searrow$ \textbf{[0]}} \\
            C ($\sfrac{5}{14}$) & A ($\sfrac{3}{14}$) & \multicolumn{2}{c}{EB ($\sfrac{3}{14} + \sfrac{1}{14}$)} & \multicolumn{2}{c}{DF ($\sfrac{1}{14} + \sfrac{1}{14}$)} \\
            & & \textbf{[1]} $\swarrow$ & $\searrow$ \textbf{[0]} & \textbf{[1]} $\swarrow$ & $\searrow$ \textbf{[0]} \\
            & & E ($\sfrac{3}{14}$) & B ($\sfrac{1}{14}$) & D ($\sfrac{1}{14}$) & F ($\sfrac{1}{14}$) \\
        \end{tabular}
    \end{table}

    Получим следующую таблицу для кодировки:
    \nopagebreak[4]
    \begin{table}[H]
        \centering
        \begin{tabular}{|r|c|c|c|c|c|c|}
            \hline
            \textbf{Символ} & C & A & E & B & D & F \\ \hline
            % \textbf{Вероятность} & $\sfrac{5}{14}$ & $\sfrac{3}{14}$ & $\sfrac{3}{14}$ & $\sfrac{1}{14}$ & $\sfrac{1}{14}$ & $\sfrac{1}{14}$ \\ \hline
            \textbf{Код} & 11 & 10 & 011 & 010 & 001 & 000 \\ \hline
        \end{tabular}
    \end{table}

    \answer%
    Исходная последовательность AAABCCCCCDEEEF кодируется следующей: $10.10.10.010.11.11.11.11.11.001.011.011.011.000$ --- 34 бита.

    Средняя длина кодового слова:
    \[
        2 \times \frac{5}{14} + 2 \times \frac{3}{14} + 3 \times \frac{3}{14} + 3 \times \frac{1}{14} + 3 \times \frac{1}{14} + 3 \times \frac{1}{14}  = \frac{34}{14} \approx \num{2.4}
    \]
\end{example}
