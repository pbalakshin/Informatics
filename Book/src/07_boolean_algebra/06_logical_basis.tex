% !TeX root = ../../main.tex
\subsection{Логический базис}\label{subsec:logical-basis}
...
\begin{figure}[H]
    ...
    \caption{Пример реализации функций И, ИЛИ, НЕ в базисе \enquote{И-НЕ}}
\end{figure}

{
    \newcommand{\cellimage}[1]{\parbox{\hsize}{\centering \includegraphics[scale=0.5]{#1}}}
    \begin{table}[!htpb]
        \caption{Пример реализации функций И, ИЛИ, НЕ в базисе \enquote{И-НЕ}}\label{tab:not-and-basis-examples}
        \begin{tabularx}{\linewidth}{|c|c|C|}
            \hline
            \thead{Функция} & \thead{Формула} & \thead{Пример} \\ \hline
            \enquote{НЕ} & $ \overline{x} = \overline{x \ast 1} = \overline{x \ast x} $ & \cellimage{7.5-nand-basis-not} \\ \hline
            \enquote{ИЛИ} & $ x_0 + x_1 = \overline{\overline{x_0} \ast \overline{x_1}} $  &  \cellimage{7.5-nand-basis-or} \\ \hline
            \enquote{И} & $ x_0 \ast x_1 = \overline{\overline{x_0 \ast x_1}} $ & \cellimage{7.5-nand-basis-and} \\ \hline
        \end{tabularx}
    \end{table}
}

\paragraph{Пример реализации функции \enquote{НЕ} в базисе \enquote{И-НЕ}}\label{par:example-not-and-basis-not}

\emph{Формула:} $ \overline{x} = \overline{x \ast 1} = \overline{x \ast x} $

\begin{figure}[H]
    \centering
    \includegraphics[scale=0.6]{7.5-nand-basis-not}
\end{figure}

\paragraph{Пример реализации функции \enquote{ИЛИ} в базисе \enquote{И-НЕ}}\label{par:example-not-and-basis-or}

\emph{Формула:} $ x_0 + x_1 = \overline{\overline{x_0} \ast \overline{x_1}} $

\begin{figure}[H]
    \centering
    \includegraphics[scale=0.6]{7.5-nand-basis-or}
\end{figure}

\paragraph{Пример реализации функции \enquote{И} в базисе \enquote{И-НЕ}}\label{par:example-not-and-basis-and}

\emph{Формула:} $ x_0 \ast x_1 = \overline{\overline{x_0 \ast x_1}} $

\begin{figure}[H]
    \centering
    \includegraphics[scale=0.6]{7.5-nand-basis-and}
\end{figure}

\subsubsection{Формы записи математических выражений}\label{subsubsec:math-notations}

Форма записи по-другому называется нотацией. \enquote{Арность операции} означает количество операндов, участвующих в операции. \\
Например: $\sqrt{A}$ (унарная), $A\times B$ (бинарная).

\paragraph{Виды нотаций}\label{par:math-notations-types}
\begin{description}
...
\textbf{Стек} --- абстрактный тип данных, представляющий собой список элементов, организованных по принципу LIFO\footnote{LIFO --- (от англ. last in --- first out, \enquote{последним пришел --- первым вышел})}.
Чаще всего принцип работы стека сравнивают со стопкой тарелок: чтобы взять вторую сверху, нужно снять верхнюю.

\subsubsection{Префиксная нотация}\label{subsubec:prefix-notation}
...
\paragraph{Особенности префиксной нотации}\label{par:prefix-notation-features}
\begin{itemize}
...
\subsubsection{Постфиксная нотация}\label{subsubsec:postfix-notation}
...
\paragraph{Особенности постфиксной нотации}\label{par:postfix-notation-features}
\begin{itemize}
    \item Не требуется скобок, если арность фиксирована.
    \item Запись выражения получается короче, чем инфиксная.
    \item Не требуется знать приоритет операций.
    \item Легко декодировать выражение с помощью стека.
    \item Успешно применяется в компиляторах, в небольшом количестве языков программирования и некоторых ЭВМ (калькуляторы \enquote{Электроника} и HP).
\end{itemize}

\paragraph{Алгоритм вычисления для постфиксной нотации}\label{par:postfix-notation-evaluation-algorithm}
\begin{enumerate}
