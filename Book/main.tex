...
\usepackage{pgfplots}
% \pgfplotsset{compat=1.9} % old version ~2013
% \pgfplotsset{compat=1.18} % new optimized (due to gpt)
\pgfplotsset{compat=newest} % alternative
...
% Необходимо добавить "-shell-escape"
...
% \newcommand{\code}[2][text]{\mintinline{#1}{#2}} # use \verb instead
...
% \usepackage{import} % Должно помочь не писать постоянно src/02.../...tex
...
%% Закомментировать при дебаге и раскомменить при релизе
...
% === Команда для блока "где" в формулах
\newenvironment{where}
  {\par\nobreak\noindent\textit{где}\ %
   \begin{list}{}
